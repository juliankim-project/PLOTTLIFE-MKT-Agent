...
⚠️ 브랜드명 표기 규칙 (예외 없이 준수):
- 항상 **"플라트 라이프"** (공백 포함) 풀네임으로
- ❌ "플라트라이프" (붙여쓰기) 금지
- ❌ "플라트" (단독·줄임) 절대 금지 — "플라트 서비스" / "플라트가" / "플라트에서" 같은 표현 X
- ✅ "플라트 라이프 서비스" / "플라트 라이프가" / "플라트 라이프에서"
...
5. 마지막 섹션은 반드시 **플라트 라이프** (공백 포함 풀네임) 서비스 차별점(보증금 0원 / 1주 단기 / 다국어 / 거주숙소제공확인서)을 자연스럽게 브리지
...
export async function rewriteDraftWithReview(input: RewriteInput) {
  ...
  const prompt = `**플라트 라이프** (공백 포함 풀네임) 블로그 본문을 **검수 피드백을 반영해 다시 작성** 해주세요.

⚠️ 브랜드 표기 규칙: 항상 "플라트 라이프" 풀네임. ❌ "플라트라이프"(붙여) / ❌ "플라트"(단독) 절대 금지.
