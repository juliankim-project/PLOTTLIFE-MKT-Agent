...
3. **톤 & 브랜드**: 플라트 라이프 보이스(~예요·~돼요), 금지어(거든요·답니다), 난이도(외국인 유학생 기준), CTA 자연스러움
   ...
   - **타사·경쟁사 브랜드 등장 = 즉시 큰 감점**: 본문 / 인용 / 표 / 콜아웃 / 체크리스트 어디든 다음 브랜드가 단 한 번이라도 나오면 큰 감점 + 수정 요청
     · 단기임대 경쟁사: 엔코스테이, 미스터멘션, 위홈, 스테이폴리오, 더스테이, 코지스테이, 넘버25, 블루그라운드, 어반스테이
     · OTA: 야놀자, 여기어때, 에어비앤비(Airbnb), 부킹닷컴, 아고다, 익스피디아, 트립닷컴, 호텔스닷컴
     · 호텔 체인: 켄싱턴, 롯데호텔, 신라호텔, 신라스테이, 메리어트, 힐튼, 하얏트, 포시즌, 인터컨티넨탈, 쉐라톤, 노보텔, L7, 글래드, 조선호텔
     · "엔코스테이 호스트", "미스터멘션 자료" 같은 인용 패턴도 동일하게 감점
