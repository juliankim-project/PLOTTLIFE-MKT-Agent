...
/**
 * Mock LLM — Phase 1
 * 플라트 라이프 브랜드 보이스와 실제 매물 맥락을 반영한 다양한 아웃라인/본문 생성.
  ...
  traffic: [
    ...
    ({ primary_keyword }) => [
      ...
      },
      {
        ...
        bullets: ["체크리스트 5개 질문", "플라트 라이프가 유리한 케이스", "추가 문의"],
      },
    ],
...
/**
 * 섹션별 풍부한 마크다운 본문 생성.
 * 플라트 라이프 브랜드 보이스 + 실제 매물 맥락 + KPI별 포맷 차별화.
 */
...
function faqAnswer(q: string, keyword: string): string {
  return `${keyword}의 경우, 이 질문은 가장 자주 받는 것 중 하나입니다. 결론부터 말씀드리면 **매물·호스트마다 다르지만 일반적인 기준**은 다음과 같습니다. 예를 들어 플라트 라이프에서는 보증금 반환이 퇴실 후 영업일 기준 3일 이내로 원칙이며, 환불 정책은 입주일 기준 단계별로 적용됩니다. 상세 답변은 각 매물 상세 페이지의 **정책 섹션**에서 확인하실 수 있습니다.`
}
...
function generateIntro(c: BlogContent): string {
  if (c.target_kpi === "conversion") {
    return `**${c.primary_keyword}** 때문에 이 글에 오셨나요? 잘 찾아오셨어요. 플라트 라이프가 직접 검증한 매물 중에서 ${c.target_audience}에게 가장 잘 맞는 방만 골라서 정리했습니다. 바로 연락 가능한 매물만 있으니 맘에 드는 방은 **"이 방 둘러보기"** 버튼으로 상세 확인하세요.`
  }
...
function generateFooter(c: BlogContent): string {
  ...
  return `${cta}\n\n---\n\n*이 글은 플라트 라이프 에디토리얼 팀이 작성했습니다. 업데이트: ${new Date().toLocaleDateString("ko-KR")}*`
}
